...
export function Sidebar({ className }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "hidden lg:flex w-72 flex-col bg-white/5 backdrop-blur-md border-r border-white/10 relative",
        className,
      )}
    >
      {/* Фоновые градиенты */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(59,130,246,0.05),transparent_50%)] pointer-events-none" />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom,rgba(147,51,234,0.03),transparent_50%)] pointer-events-none" />

      {/* Заголовок */}
      <div className="flex items-center justify-center p-6 border-b border-white/10 relative z-10">
        <div className="flex flex-col items-center gap-3">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500/20 to-blue-600/20 flex items-center justify-center border border-blue-500/30 backdrop-blur-sm">
            <span className="text-2xl font-bold text-blue-400">107</span>
          </div>
          <div className="text-center">
            <h2 className="text-lg font-bold text-white tracking-tight">ОККИПд-107</h2>
            <p className="text-xs text-gray-400 mt-1">ZorahM-LMS</p>
          </div>
        </div>
      </div>
